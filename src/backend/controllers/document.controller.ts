...
export const DocumentController = {
  ...
  async download(request: NextRequest) {
    const auth = await authenticate(request);
    if (!isAuthenticated(auth)) return auth;
    ...
    try {
      ...
      if (!body.cycleId) return error("cycleId is required");

      const template = getTemplate(body.templateId);
      if (!template) return error(`Template "${body.templateId}" not found`, 404);

      const cycle = await SchoolCycleRepository.findById(body.cycleId);
      if (!cycle) return error("School cycle not found", 404);

      const { folioNumber, fullFolio } = await FolioService.generateFolio(
        template.docTypeId, body.cycleId
      );

      const ctx: TemplateContext = {
        program: { id: 0, name: "Programa", isSocialService: false },
        cycle: {
          id: cycle.idCycle, cycle: cycle.cycle, description: cycle.description,
      ...
      if (body.studentId) {
        ...
        const program = await ProgramRepository.findById(studentRow.programId);
        if (program) {
          ctx.program = { id: program.idProgram, name: program.name, isSocialService: program.isSocialService };
        }
        ctx.student = toSafeStudent(studentRow, userRow, "STUDENT");
        ctx.gender = userRow.gender as Gender;
      }
      ...
      if (body.useAI) {
        ...
        if (aiRequest) {
          const aiResult = await LlmService.generateBody(aiRequest);
          if (aiResult.aiUsed) aiBodyParagraphs = aiResult.paragraphs;
        }
      }

      const pdfBytes = await DocumentService.generate(template, ctx, { aiBodyParagraphs });
