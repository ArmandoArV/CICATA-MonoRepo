...
interface RegisterSubjectModalProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
export function RegisterSubjectModal({
  open,
  onClose,
  onSuccess,
}: RegisterSubjectModalProps) {
  const { token } = useAuth();
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Registrar Materia" maxWidth="max-w-2xl">
      {step === 1 && (
        <>
          <AnimateIn delay={0}>
